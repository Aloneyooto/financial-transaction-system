...
import com.alone.counter.bean.res.StockInfo;
 ...
 * @Author: Alone
 * @CreateTime: 2020-10-31 15:42
 * @Description: 一次性加载所有股票名称
 */
@Log4j2
@Component
public class StockCache {
    ...
    //实现倒排索引的数据结构
    //MultiMap等于Map<String, List<StockInfo> >
    //6 --> 600086, 600025
    ...
    private List<String> splitData(String format) {
        //payh ->
        //p pa pay payh
        //a ay ayh
        //y yh
        ...
        int outLength = format.length();
        for (int i = 0; i < outLength; i++) {
            int inLength = outLength + 1;
            for (int j = i + 1; j < inLength; j++) {
                list.add(format.substring(i, j));
            }
        }
        return list;
    }
